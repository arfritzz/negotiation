...
the same goals, there are differences between the implimentation of the
two with benefits for defining the language in either way. 
Functional definitions produce whatever value is desired (ie a number).
A way to think about relational definitions is as simple as thinking about
a relation between expression and their values. Relational definitions produce
a Prop. However, it may be useful to prove that the two definitions are
equal?

Things that are inhearent to functional definitions:
...
Things that are inhearent to relational definitions:

\begin{itemize}
  \squash
\item Inductive
\item Would return a proposition
\end{itemize}
...
\item p_2 $=$ \mathsf{USM} {\hsh{virus checker}, \hsh {signature file}}, additional
  evidence
\end{itemize}
